...
set -eu
cd "$(dirname "$0")"
docker rm -f msp-sandbox msp-sandbox-host >/dev/null 2>&1 || true
docker network inspect msp-sandbox >/dev/null 2>&1 || docker network create msp-sandbox >/dev/null
docker build -q -t msp-sandbox . >/dev/null
docker build -q -t msp-sandbox-host -f Dockerfile.host . >/dev/null
docker run -d --name msp-sandbox --privileged --cgroupns=host \
  -v /sys/fs/cgroup:/sys/fs/cgroup:rw --tmpfs /run --tmpfs /run/lock \
  --hostname sandbox-box --network msp-sandbox msp-sandbox >/dev/null
docker run -d --name msp-sandbox-host --privileged --cgroupns=host \
  -v /sys/fs/cgroup:/sys/fs/cgroup:rw --tmpfs /run --tmpfs /run/lock \
  --hostname sandbox-host --network msp-sandbox msp-sandbox-host >/dev/null
...
i=0; until docker exec msp-sandbox-host systemctl is-system-running 2>/dev/null | grep -qE 'running|degraded'; do i=$((i+1)); [ $i -gt 20 ] && { echo "host did not boot"; exit 1; }; sleep 1; done
docker exec msp-sandbox-host mkdir -p /var/log/msp
echo "sandbox up (box + host)"
