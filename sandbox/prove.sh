...
cd "$(dirname "$0")/.." || exit 1
./bin/lint || { echo "PROOF FAILED: lint"; exit 1; }
...
grep -q 'ALL PASS' /tmp/sbx.out || { echo "PROOF FAILED: tests"; exit 1; }
./sandbox/offboard.sh > /tmp/off.out 2>&1
grep -E 'FAIL|----' /tmp/off.out; echo "pass=$(grep -c '^PASS' /tmp/off.out)"
grep -q 'ALL PASS' /tmp/off.out || { echo "PROOF FAILED: offboard"; exit 1; }
echo "PROOF OK"
